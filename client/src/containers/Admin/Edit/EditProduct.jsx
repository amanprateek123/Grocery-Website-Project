...
const AddProduct = (props) => {
    ...
    const addIMG = (i) => {
        ...
        let _schema = schema.skus[0].images[0];
    ...
    const addATTR = (i) => {
        ...
        let _schema = schema.skus[0].attributes[0];
    ...
    const getProduct = (e) => {
        e.preventDefault();
        fetch(`/get-products?id=${productId}`).then(product => product.json())
            .then(({ products: [_product] }) => {
                if (_product) {
                    setSelectedCat({
                        department: _product.category.id,
                        parentCategory: _product.category.parentCategory.id,
                        category: _product.category.parentCategory.department.id,
                    })
                    setSelectedCat({
                        department: categories.find(dept => dept.id == _product.category.parentCategory.department.id),
                        parentCategory: categories.find(dept => dept.id == _product.category.parentCategory.department.id)
                            .parentCategories.find(pc => pc.id == _product.category.parentCategory.id),
                        category: categories.find(dept => dept.id == _product.category.parentCategory.department.id)
                            .parentCategories.find(pc => pc.id == _product.category.parentCategory.id)
                            .categories.find(cat => cat.id == _product.category.id),
                    });
                    setProduct(_product)
                }
                else {
                            ...
                            <div className="row">
                                <div className="col">
                                    <div className="d-flex mb-4">
                                        <h2 className="mb-2 text-muted">Edit Product</h2>
                                        <div>
                                            <Button color="secondary" className="mr-4" type="button" onClick={(e) => setProduct(null)}>Cancel</Button>
                                            <Button variant="contained" color="primary" type="submit">Save</Button>
                                        </div>
                                    </div>
                                    <div className="info">
                                        <p>
                                            Old Product will be replaced with this One.
                                </p>
                                ...
                                <div className="col">
                                    ...
                                    {product.skus.map((sku, i) => (
                                        <div key={sku + i} className="sku">
                                            ...
                                            <div className="form-group">
                                                <TextField required type="number" value={sku.weight} name="weight" onChange={(e) => handleChangeSKU(e, i)} label="weight" InputLabelProps={{ shrink: Boolean(sku.weight) }} />
                                                <TextField required type="number" value={sku.extraCharges} name="extraCharges" onChange={(e) => handleChangeSKU(e, i)} label="extraCharges" InputLabelProps={{ shrink: Boolean(sku.extraCharges) }} />
                                            </div>
                                            <div className="form-group">
                                            ...
                                            <div className="images ml-2">
                                                ...
                                                <div className="imgs">
                                                    ...
                                                    <div className="images">
                                                        {imgUrls[i] ? imgUrls[i].map(url => <img width={100} src={url} />) : null}
                                                    </div>
                                                </div>
                                            </div>
