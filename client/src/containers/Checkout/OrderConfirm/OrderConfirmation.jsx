...
function OrderConfirmation(props) {
    ...
    try {
        ship = JSON.parse(props.data.shippingAddress)
    }
    catch (err) {
        ship = null;
    }
            ...
            <Paper className="container mt-5 mb-5" id="confi">
                    ...
                    <p style={{ fontSize: '17px', margin: '20px 40px' }}>Thanks for shopping! Your Order hasn't shipped yet, but we'll send you an email while it does.</p>
                    {
                        props.paymentData ?
                            <div className="payment-data">
                                ...
                                <p><b>{props.paymentData.id}</b> is your transaction reference number. please keep it safe for reference.</p>
                            </div>
                            : null
                    }
                </div>
                {
                    props.data?.id && ship ?
                        ...
                        : <NavLink to={`/orders`}><button className="order_btn">View Orders</button></NavLink>
                }
